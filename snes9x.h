...
#define STREAM					snes4nacl::FS::Handle*
#define READ_STREAM(p, l, s)	s->fread(p, 1, l)
#define WRITE_STREAM(p, l, s)	s->fwrite(p, 1, l)
#define GETS_STREAM(p, l, s)	s->fgets(p, l)
#define GETC_STREAM(s)			s->fgetc()
#define OPEN_STREAM(f, m)		snes4nacl::Instance::get()->getEmulator().getFS().fopen(f, m)
#define REOPEN_STREAM(f, m)		snes4nacl::Instance::get()->getEmulator().getFS().fdopen(f, m)
#define FIND_STREAM(f)			f->ftell()
#define REVERT_STREAM(f, o, s)	f->fseek(o, s)
#define CLOSE_STREAM(s)			s->fclose()
#else
#define STREAM					FILE *
