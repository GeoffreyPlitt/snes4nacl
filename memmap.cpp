...
bool8 CMemory::LoadSRAM (const char *filename)
{
	...
	if (Multi.cartType && Multi.sramSizeB)
	{
		...
		file = OPEN_STREAM(S9xGetFilename(".srm", SRAM_DIR), "rb");
		if (file)
		{
			len = READ_STREAM((char *) Multi.sramB, 0x10000, file);
			CLOSE_STREAM(file);
			if (len - size == 512)
	...
	if (size)
	{
		file = OPEN_STREAM(sramName, "rb");
		if (file)
		{
			len = READ_STREAM((char *) SRAM, 0x20000, file);
			CLOSE_STREAM(file);
			if (len - size == 512)
